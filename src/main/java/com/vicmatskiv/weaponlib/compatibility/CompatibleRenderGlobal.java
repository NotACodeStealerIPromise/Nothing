...
public class CompatibleRenderGlobal extends RenderGlobal implements IWorldAccess {
   ...
   public CompatibleRenderGlobal(Minecraft p_i1249_1_) {
      super(p_i1249_1_);
      this.mc = p_i1249_1_;
   ...
   public void loadRenderers() {
      if(this.theWorld != null) {
         ...
         for(l = 0; l < this.renderChunksWide; ++l) {
            for(int entitylivingbase = 0; entitylivingbase < this.renderChunksTall; ++entitylivingbase) {
               for(int j1 = 0; j1 < this.renderChunksDeep; ++j1) {
                  this.worldRenderers[(j1 * this.renderChunksTall + entitylivingbase) * this.renderChunksWide + l] = new WorldRenderer(this.theWorld, this.tileEntities, l * 16, entitylivingbase * 16, j1 * 16, this.glRenderListBase + j);
                  if(this.occlusionEnabled) {
                     this.worldRenderers[(j1 * this.renderChunksTall + entitylivingbase) * this.renderChunksWide + l].glOcclusionQuery = this.glOcclusionQueryBase.get(k);
                  }

                  this.worldRenderers[(j1 * this.renderChunksTall + entitylivingbase) * this.renderChunksWide + l].isWaitingOnOcclusionQuery = false;
                  this.worldRenderers[(j1 * this.renderChunksTall + entitylivingbase) * this.renderChunksWide + l].isVisible = true;
                  this.worldRenderers[(j1 * this.renderChunksTall + entitylivingbase) * this.renderChunksWide + l].isInFrustum = true;
                  this.worldRenderers[(j1 * this.renderChunksTall + entitylivingbase) * this.renderChunksWide + l].chunkIndex = k++;
                  this.worldRenderers[(j1 * this.renderChunksTall + entitylivingbase) * this.renderChunksWide + l].markDirty();
                  this.sortedWorldRenderers[(j1 * this.renderChunksTall + entitylivingbase) * this.renderChunksWide + l] = this.worldRenderers[(j1 * this.renderChunksTall + entitylivingbase) * this.renderChunksWide + l];
                  this.worldRenderersToUpdate.add(this.worldRenderers[(j1 * this.renderChunksTall + entitylivingbase) * this.renderChunksWide + l]);
                  j += 3;
               }
            }
         }
   ...
   public void renderEntities(EntityLivingBase p_147589_1_, ICamera p_147589_2_, float p_147589_3_) {
      ...
      if(this.renderEntitiesStartupCounter > 0) {
         ...
         for(i = 0; i < list.size(); ++i) {
            ...
            if(entity.shouldRenderInPass(pass)) {
               boolean tile = entity.isInRangeToRender3d(d0, d1, d2) && (entity.ignoreFrustumCheck || p_147589_2_.isBoundingBoxInFrustum(entity.boundingBox) || entity.riddenByEntity == this.mc.thePlayer);
               if(!tile && entity instanceof EntityLiving) {
                  EntityLiving entityliving = (EntityLiving)entity;
                  if(entityliving.getLeashed() && entityliving.getLeashedToEntity() != null) {
                     Entity entity1 = entityliving.getLeashedToEntity();
                     tile = p_147589_2_.isBoundingBoxInFrustum(entity1.boundingBox);
                  }
               }

               if(tile && (entity != this.mc.renderViewEntity || this.mc.gameSettings.thirdPersonView != 0 || this.mc.renderViewEntity.isPlayerSleeping()) && this.theWorld.blockExists(MathHelper.floor_double(entity.posX), 0, MathHelper.floor_double(entity.posZ))) {
                  ++this.countEntitiesRendered;
                  RenderManager.instance.renderEntitySimple(entity, p_147589_3_);
               }
            }
         ...
         for(i = 0; i < this.tileEntities.size(); ++i) {
            TileEntity var24 = (TileEntity)this.tileEntities.get(i);
            if(var24.shouldRenderInPass(pass) && p_147589_2_.isBoundingBoxInFrustum(var24.getRenderBoundingBox())) {
               TileEntityRendererDispatcher.instance.renderTileEntity(var24, p_147589_3_);
            }
         }
   ...
   public int sortAndRender(EntityLivingBase p_72719_1_, int p_72719_2_, double p_72719_3_) {
      ...
      double var41 = p_72719_1_.lastTickPosX + (p_72719_1_.posX - p_72719_1_.lastTickPosX) * p_72719_3_;
      ...
      if(this.occlusionEnabled && this.mc.gameSettings.advancedOpengl && !this.mc.gameSettings.anaglyph && p_72719_2_ == 0) {
         ...
         int l1;
         for(l1 = b0; l1 < l; ++l1) {
            this.sortedWorldRenderers[l1].isVisible = true;
         }
               ...
               if(this.sortedWorldRenderers[j1].skipAllRenderPasses()) {
                  ...
                  if(this.sortedWorldRenderers[j1].isInFrustum && !this.sortedWorldRenderers[j1].isWaitingOnOcclusionQuery) {
                     ...
                     if(this.cloudTickCounter % k1 == j1 % k1) {
                        ...
                        float f3 = (float)((double)worldrenderer1.posXMinus - var41);
                        ...
                        ARBOcclusionQuery.glBeginQueryARB('褔', this.sortedWorldRenderers[j1].glOcclusionQuery);
   ...
   private void checkOcclusionQueryResult(int p_72720_1_, int p_72720_2_) {
      for(int k = p_72720_1_; k < p_72720_2_; ++k) {
         if(this.sortedWorldRenderers[k].isWaitingOnOcclusionQuery) {
            this.occlusionResult.clear();
            ARBOcclusionQuery.glGetQueryObjectuARB(this.sortedWorldRenderers[k].glOcclusionQuery, '衧', this.occlusionResult);
            if(this.occlusionResult.get(0) != 0) {
               this.sortedWorldRenderers[k].isWaitingOnOcclusionQuery = false;
               this.occlusionResult.clear();
               ARBOcclusionQuery.glGetQueryObjectuARB(this.sortedWorldRenderers[k].glOcclusionQuery, '衦', this.occlusionResult);
   ...
   private int renderSortedRenderers(int p_72724_1_, int p_72724_2_, int p_72724_3_, double p_72724_4_) {
      ...
      for(int entitylivingbase = i1; entitylivingbase != j1; entitylivingbase += b0) {
         if(p_72724_3_ == 0) {
            ++this.renderersLoaded;
            if(this.sortedWorldRenderers[entitylivingbase].skipRenderPass[p_72724_3_]) {
               ++this.renderersSkippingRenderPass;
            } else if(!this.sortedWorldRenderers[entitylivingbase].isInFrustum) {
               ++this.renderersBeingClipped;
            } else if(this.occlusionEnabled && !this.sortedWorldRenderers[entitylivingbase].isVisible) {
         ...
         if(!this.sortedWorldRenderers[entitylivingbase].skipRenderPass[p_72724_3_] && this.sortedWorldRenderers[entitylivingbase].isInFrustum && (!this.occlusionEnabled || this.sortedWorldRenderers[entitylivingbase].isVisible)) {
            int d3 = this.sortedWorldRenderers[entitylivingbase].getGLCallListForPass(p_72724_3_);
            if(d3 >= 0) {
               this.glRenderLists.add(this.sortedWorldRenderers[entitylivingbase]);
               ++l;
            }
         }
      }
      ...
      double d1 = var22.lastTickPosY + (var22.posY - var22.lastTickPosY) * p_72724_4_;
      double d2 = var22.lastTickPosZ + (var22.posZ - var22.lastTickPosZ) * p_72724_4_;
      int i2 = 0;
      ...
      for(j2 = 0; j2 < this.glRenderLists.size(); ++j2) {
         WorldRenderer i3 = (WorldRenderer)this.glRenderLists.get(j2);
         k2 = -1;

         for(l2 = 0; l2 < i2; ++l2) {
            if(this.allRenderLists[l2].rendersChunk(i3.posXMinus, i3.posYMinus, i3.posZMinus)) {
               k2 = l2;
            }
         }
         ...
         if(k2 < 0) {
            k2 = i2++;
            this.allRenderLists[k2].setupRenderList(i3.posXMinus, i3.posYMinus, i3.posZMinus, var23, d1, d2);
         }

         this.allRenderLists[k2].addGLRenderList(i3.getGLCallListForPass(p_72724_3_));
      }

      j2 = MathHelper.floor_double(var23);
      int var24 = MathHelper.floor_double(d2);
      k2 = j2 - (j2 & 1023);
      l2 = var24 - (var24 & 1023);
   ...
   public void renderSky(float p_72714_1_) {
      ...
      if((skyProvider = this.mc.theWorld.provider.getSkyRenderer()) != null) {
         ...
         if(this.mc.theWorld.provider.dimensionId == 1) {
            ...
            Tessellator vec3 = Tessellator.instance;
            ...
            for(int f1 = 0; f1 < 6; ++f1) {
               ...
               vec3.startDrawingQuads();
               vec3.setColorOpaque_I(2631720);
               vec3.addVertexWithUV(-100.0D, -100.0D, -100.0D, 0.0D, 0.0D);
               vec3.addVertexWithUV(-100.0D, -100.0D, 100.0D, 0.0D, 16.0D);
               vec3.addVertexWithUV(100.0D, -100.0D, 100.0D, 16.0D, 16.0D);
               vec3.addVertexWithUV(100.0D, -100.0D, -100.0D, 16.0D, 0.0D);
               vec3.draw();
               GL11.glPopMatrix();
            }
            ...
            GL11.glDisable(3553);
            Vec3 var24 = this.theWorld.getSkyColor(this.mc.renderViewEntity, p_72714_1_);
            float var25 = (float)var24.xCoord;
            float f2 = (float)var24.yCoord;
            float f3 = (float)var24.zCoord;
            float f6;
            if(this.mc.gameSettings.anaglyph) {
               float tessellator1 = (var25 * 30.0F + f2 * 59.0F + f3 * 11.0F) / 100.0F;
               float afloat = (var25 * 30.0F + f2 * 70.0F) / 100.0F;
               f6 = (var25 * 30.0F + f3 * 70.0F) / 100.0F;
               var25 = tessellator1;
               f2 = afloat;
               f3 = f6;
            }

            GL11.glColor3f(var25, f2, f3);
            ...
            OpenGlHelper.glBlendFunc(770, 771, 1, 0);
            RenderHelper.disableStandardItemLighting();
            float[] var27 = this.theWorld.provider.calcSunriseSunsetColors(this.theWorld.getCelestialAngle(p_72714_1_), p_72714_1_);
            float f7;
            float f8;
            float f9;
            float f10;
            ...
            if(var27 != null) {
               ...
               var26.setColorRGBA_F(f6, f7, f8, var27[3]);
               var26.addVertex(0.0D, 100.0D, 0.0D);
               byte l = 16;
               var26.setColorRGBA_F(var27[0], var27[1], var27[2], 0.0F);

               for(i1 = 0; i1 <= l; ++i1) {
                  k = (float)i1 * 3.1415927F * 2.0F / (float)l;
                  f14 = MathHelper.sin(k);
                  f15 = MathHelper.cos(k);
                  var26.addVertex((double)(f14 * 120.0F), (double)(f15 * 120.0F), (double)(-f15 * 40.0F * var27[3]));
               }
            ...
            var26.addVertexWithUV((double)(-f10), 100.0D, (double)(-f10), 0.0D, 0.0D);
            var26.addVertexWithUV((double)f10, 100.0D, (double)(-f10), 1.0D, 0.0D);
            var26.addVertexWithUV((double)f10, 100.0D, (double)f10, 1.0D, 1.0D);
            var26.addVertexWithUV((double)(-f10), 100.0D, (double)f10, 0.0D, 1.0D);
            var26.draw();
            f10 = 20.0F;
            ...
            int var29 = var28 % 4;
            i1 = var28 / 4 % 2;
            f14 = (float)(var29 + 0) / 4.0F;
            f15 = (float)(i1 + 0) / 2.0F;
            float f16 = (float)(var29 + 1) / 4.0F;
            float f17 = (float)(i1 + 1) / 2.0F;
            var26.startDrawingQuads();
            var26.addVertexWithUV((double)(-f10), -100.0D, (double)f10, (double)f16, (double)f17);
            var26.addVertexWithUV((double)f10, -100.0D, (double)f10, (double)f14, (double)f17);
            var26.addVertexWithUV((double)f10, -100.0D, (double)(-f10), (double)f14, (double)f15);
            var26.addVertexWithUV((double)(-f10), -100.0D, (double)(-f10), (double)f16, (double)f15);
            var26.draw();
            GL11.glDisable(3553);
            ...
            if(d0 < 0.0D) {
               ...
               var26.addVertex((double)(-f8), (double)f9, (double)f8);
               var26.addVertex((double)f8, (double)f9, (double)f8);
               var26.addVertex((double)f8, (double)f10, (double)f8);
               var26.addVertex((double)(-f8), (double)f10, (double)f8);
               var26.addVertex((double)(-f8), (double)f10, (double)(-f8));
               var26.addVertex((double)f8, (double)f10, (double)(-f8));
               var26.addVertex((double)f8, (double)f9, (double)(-f8));
            ...
            if(this.theWorld.provider.isSkyColored()) {
               GL11.glColor3f(var25 * 0.2F + 0.04F, f2 * 0.2F + 0.04F, f3 * 0.6F + 0.1F);
            } else {
               GL11.glColor3f(var25, f2, f3);
            }
         ...
         if(this.mc.theWorld.provider.isSurfaceWorld()) {
            if(this.mc.gameSettings.fancyGraphics) {
               ...
               if(this.mc.gameSettings.anaglyph) {
                  ...
                  float d2 = (f2 * 30.0F + f3 * 70.0F) / 100.0F;
               ...
               f5 = 4.8828125E-4F;
               double d21 = (double)((float)this.cloudTickCounter + p_72718_1_);
               double d0 = this.mc.renderViewEntity.prevPosX + (this.mc.renderViewEntity.posX - this.mc.renderViewEntity.prevPosX) * (double)p_72718_1_ + d21 * 0.029999999329447746D;
   ...
   public boolean updateRenderers(EntityLivingBase p_72716_1_, boolean p_72716_2_) {
      ...
      for(k1 = b0 - 1; k1 >= 0; --k1) {
         worldrenderer1 = aworldrenderer[k1];
         if(worldrenderer1 != null) {
            if(!worldrenderer1.isInFrustum && k1 != b0 - 1) {
      ...
      for(i1 = this.worldRenderersToUpdate.size(); k1 != i1; ++k1) {
         worldrenderer1 = (WorldRenderer)this.worldRenderersToUpdate.get(k1);
         if(worldrenderer1 != null) {
            boolean flag1 = false;
   ...
   public void spawnParticle(String p_72708_1_, final double p_72708_2_, final double p_72708_4_, final double p_72708_6_, double p_72708_8_, double p_72708_10_, double p_72708_12_) {
      try {
         ...
         CrashReport crashreport = CrashReport.makeCrashReport(var17, "Exception while adding particle");
   ...
   public EntityFX doSpawnParticle(String p_72726_1_, double p_72726_2_, double p_72726_4_, double p_72726_6_, double p_72726_8_, double p_72726_10_, double p_72726_12_) {
      if(this.mc != null && this.mc.renderViewEntity != null && this.mc.effectRenderer != null) {
         ...
         if(p_72726_1_.equals("hugeexplosion")) {
            this.mc.effectRenderer.addEffect((EntityFX)(entityfx = new EntityHugeExplodeFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_)));
         } else if(p_72726_1_.equals("largeexplode")) {
            this.mc.effectRenderer.addEffect((EntityFX)(entityfx = new EntityLargeExplodeFX(this.renderEngine, this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_)));
         } else if(p_72726_1_.equals("fireworksSpark")) {
            this.mc.effectRenderer.addEffect((EntityFX)(entityfx = new EntityFireworkSparkFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_, this.mc.effectRenderer)));
         }
         ...
         if(entityfx != null) {
            ...
            if(d6 * d6 + d7 * d7 + d8 * d8 > d9 * d9) {
               ...
               if(p_72726_1_.equals("bubble")) {
                  ...
                  entityfx = new EntitySpellParticleFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_);
                  ((EntitySpellParticleFX)entityfx).setBaseSpellTextureIndex(144);
                  float k = this.theWorld.rand.nextFloat() * 0.5F + 0.35F;
                  ...
                  if(p_72726_1_.startsWith("iconcrack_")) {
                     ...
                     if(astring.length > 2) {
                        ...
                        entityfx = new EntityBreakingFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_, Item.getItemById(block), k1);
                     } else {
                        entityfx = new EntityBreakingFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_, Item.getItemById(block), 0);
                     }
                  } else {
                     Block block1;
                     if(p_72726_1_.startsWith("blockcrack_")) {
                        ...
                        k1 = Integer.parseInt(astring[2]);
                        entityfx = (new EntityDiggingFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_, block1, k1)).applyRenderColor(k1);
                     } else if(p_72726_1_.startsWith("blockdust_")) {
                        astring = p_72726_1_.split("_", 3);
                        block1 = Block.getBlockById(Integer.parseInt(astring[1]));
                        k1 = Integer.parseInt(astring[2]);
                        entityfx = (new EntityBlockDustFX(this.theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_, block1, k1)).applyRenderColor(k1);
                     }
                  }
               }
   ...
   public void playAuxSFX(EntityPlayer p_72706_1_, int p_72706_2_, int p_72706_3_, int p_72706_4_, int p_72706_5_, int p_72706_6_) {
      ...
      case 2000:
         ...
         for(int var43 = 0; var43 < 10; ++var43) {
            double var44 = random.nextDouble() * 0.2D + 0.01D;
            double var45 = d1 + (double)j2 * 0.01D + (random.nextDouble() - 0.5D) * (double)j1 * 0.5D;
            d4 = d2 + (random.nextDouble() - 0.5D) * 0.5D;
            d13 = d9 + (double)j1 * 0.01D + (random.nextDouble() - 0.5D) * (double)j2 * 0.5D;
            d5 = (double)j2 * var44 + random.nextGaussian() * 0.01D;
            d6 = -0.03D + random.nextGaussian() * 0.01D;
            d7 = (double)j1 * var44 + random.nextGaussian() * 0.01D;
            this.spawnParticle("smoke", var45, d4, d13, d5, d6, d7);
         }
      ...
      case 2002:
         ...
         for(l2 = 0; l2 < 100; ++l2) {
            ...
            d13 = random.nextDouble() * 3.141592653589793D * 2.0D;
            d5 = Math.cos(d13) * d4;
            d6 = 0.01D + random.nextDouble() * 0.5D;
            d7 = Math.sin(d13) * d4;
            EntityFX var46 = this.doSpawnParticle(s1, d0 + d5 * 0.1D, d1 + 0.3D, d2 + d7 * 0.1D, d5, d6, d7);
            if(var46 != null) {
               float f4 = 0.75F + random.nextFloat() * 0.25F;
               var46.setRBGColorF(f * f4, f1 * f4, f2 * f4);
               var46.multiplyVelocity((float)d4);
            }
         }
      ...
      case 2003:
         ...
         for(d3 = 0.0D; d3 < 6.283185307179586D; d3 += 0.15707963267948966D) {
            this.spawnParticle("portal", d0 + Math.cos(d3) * 5.0D, d1 - 0.4D, d2 + Math.sin(d3) * 5.0D, Math.cos(d3) * -5.0D, 0.0D, Math.sin(d3) * -5.0D);
            this.spawnParticle("portal", d0 + Math.cos(d3) * 5.0D, d1 - 0.4D, d2 + Math.sin(d3) * 5.0D, Math.cos(d3) * -7.0D, 0.0D, Math.sin(d3) * -7.0D);
         }
      ...
      case 2006:
         ...
         if(block.getMaterial() != Material.air) {
            d3 = (double)Math.min(0.2F + (float)p_72706_6_ / 15.0F, 10.0F);
            if(d3 > 2.5D) {
               d3 = 2.5D;
            }
            ...
            for(int i2 = 0; i2 < l1; ++i2) {
               float f3 = MathHelper.randomFloatClamp(random, 0.0F, 6.2831855F);
