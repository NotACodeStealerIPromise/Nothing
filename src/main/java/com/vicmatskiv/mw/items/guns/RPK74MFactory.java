...
public class RPK74MFactory implements GunFactory {
   public Item createGun(CommonProxy commonProxy) {
      return (new Weapon.Builder()).withModId("mw").withName("RPK74M").withFireRate(0.6F).withRecoil(4.0F).withZoom(0.9F).withMaxShots(new int[]{Integer.MAX_VALUE, 3, 1}).withShootSound("RPK74M").withSilencedShootSound("AKsilenced").withReloadSound("AKReload").withUnloadSound("AKUnload").withReloadingTime(45L).withCrosshair("gun").withCrosshairRunning("Running").withCrosshairZoomed("Sight").withFlashIntensity(1.0F).withFlashScale(() -> {
         return Float.valueOf(0.8F);
      }).withFlashOffsetX(() -> {
         return Float.valueOf(0.1F);
      }).withFlashOffsetY(() -> {
         return Float.valueOf(0.1F);
      }).withInaccuracy(1.0F).withCreativeTab(ModernWarfareMod.ShotgunsTab).withCrafting(CraftingComplexity.MEDIUM, new Object[]{CommonProxy.SteelPlate, CommonProxy.MetalComponents, CommonProxy.MiniSteelPlate}).withInformationProvider((stack) -> {
         return Arrays.asList(new String[]{"Type: Light machine gun", "Damage: 7.2", "Caliber: 7.62x39mm", "Magazines:", "75rnd 7.62x39mm Drum Magazine", "Fire Rate: Auto"});
      ...
      })).withTextureNames(new String[]{"RPK74M"}).withRenderer((new WeaponRenderer.Builder()).withModId("mw").withModel(new RPK74M()).withEntityPositioning((itemStack) -> {
