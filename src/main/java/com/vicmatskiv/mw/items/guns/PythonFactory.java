...
public class PythonFactory implements GunFactory {
   public Item createGun(CommonProxy commonProxy) {
      return (new Weapon.Builder()).withModId("mw").withName("Python").withAmmoCapacity(6).withFireRate(0.2F).withRecoil(5.0F).withZoom(0.9F).withMaxShots(new int[]{1}).withShootSound("Python").withReloadSound("MagnumReload").withReloadingTime(25L).withCrosshair("gun").withCrosshairRunning("Running").withCrosshairZoomed("Sight").withFlashIntensity(1.0F).withFlashScale(() -> {
         ...
         return Arrays.asList(new String[]{"Type: Revolver", "Damage: 6", "Ammo: .357 Bullet", "Fire Rate: Semi"});
      ...
      }).withTextureNames(new String[]{"Python", "MagnumGold"}).withRenderer((new WeaponRenderer.Builder()).withModId("mw").withModel(new Python()).withEntityPositioning((itemStack) -> {
         ...
         GL11.glScaled(0.550000011920929D, 0.550000011920929D, 0.550000011920929D);
         GL11.glTranslatef(-1.1F, -0.76F, 1.5F);
      }, 250L, 100L)}).withFirstPersonCustomPositioningReloading(AuxiliaryAttachments.PythonCase.getRenderablePart(), new Transition[]{new Transition((renderContext) -> {
