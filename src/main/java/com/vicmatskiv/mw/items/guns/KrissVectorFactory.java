...
public class KrissVectorFactory implements GunFactory {
   public Item createGun(CommonProxy commonProxy) {
      return (new Weapon.Builder()).withModId("mw").withName("KrissVector").withFireRate(0.75F).withRecoil(3.0F).withZoom(0.9F).withMaxShots(new int[]{Integer.MAX_VALUE, 3, 1}).withShootSound("Vector").withSilencedShootSound("VectorSilenced").withReloadSound("StandardReload").withUnloadSound("Unload").withReloadingTime(45L).withCrosshair("gun").withCrosshairRunning("Running").withCrosshairZoomed("Sight").withFlashIntensity(1.0F).withFlashScale(() -> {
         return Float.valueOf(0.8F);
      }).withFlashOffsetX(() -> {
         return Float.valueOf(0.2F);
      }).withFlashOffsetY(() -> {
         return Float.valueOf(0.2F);
      }).withInaccuracy(2.0F).withCrafting(CraftingComplexity.LOW, new Object[]{CommonProxy.SteelPlate, CommonProxy.MiniSteelPlate, "ingotSteel", CommonProxy.BigSteelPlate}).withCreativeTab(ModernWarfareMod.SMGTab).withInformationProvider((stack) -> {
         ...
         GL11.glScaled(1.5D, 1.5D, 1.5D);
      }).withTextureNames(new String[]{"KrissVector", "Electric"}).withRenderer((new WeaponRenderer.Builder()).withModId("mw").withModel(new KrissVector()).withEntityPositioning((itemStack) -> {
         GL11.glScaled(0.30000001192092896D, 0.30000001192092896D, 0.30000001192092896D);
