...
public class AK74Factory implements GunFactory {
   public Item createGun(CommonProxy commonProxy) {
      return (new Weapon.Builder()).withModId("mw").withName("AK-74").withFireRate(0.5F).withRecoil(3.0F).withZoom(0.9F).withMaxShots(new int[]{Integer.MAX_VALUE, 1}).withShootSound("AK74").withSilencedShootSound("AKsilenced").withReloadSound("AKReload").withUnloadSound("akunload").withReloadingTime(45L).withCrosshair("gun").withCrosshairRunning("Running").withCrosshairZoomed("Sight").withFlashIntensity(1.0F).withFlashScale(() -> {
         return Float.valueOf(0.8F);
      }).withFlashOffsetX(() -> {
         return Float.valueOf(0.1F);
      }).withFlashOffsetY(() -> {
         return Float.valueOf(0.13F);
      }).withCrafting(CraftingComplexity.MEDIUM, new Object[]{CommonProxy.SteelPlate, CommonProxy.MiniSteelPlate, CommonProxy.MetalComponents, CompatibleBlocks.PLANK}).withCreativeTab(ModernWarfareMod.AssaultRiflesTab).withInformationProvider((stack) -> {
      ...
      })).withTextureNames(new String[]{"AK74"}).withRenderer((new WeaponRenderer.Builder()).withModId("mw").withModel(new AK74()).withEntityPositioning((itemStack) -> {
